...
def main():
    ...
    while viewer.is_available() and not exit_signal:
        if zed.grab(runtime_params) == sl.ERROR_CODE.SUCCESS:
            ...
            # Output Json
            results_py = {'time':[], 'name':[], 'id':[], 'position':[],'bounding box':[]}
            if objects.is_new:
                # Count the number of objects detected
                print("{} Object(s) detected".format(len(objects.object_list)))

                if len(objects.object_list):
                    results_py['time'].append(objects.timestamp.data_ns)
                    for object in objects.object_list:
                        results_py['name'].append(object.label.value)
                        results_py['id'].append(object.unique_object_id)
                        results_py['position'].append(object.position.tolist())
                        results_py['bounding box'].append(object.bounding_box_2d.tolist())
            
            results_js=json.dumps(results_py)
            print(results_js)
